...
for network_type in range(2):
		...
		plt.xlim((0, 300))
		plt.xlabel("Time (s)") 
		plt.ylabel("Average Packet Length (Byte)") 
		plt.grid()
		plt.title(network_list[network_type] + ", Real Person")
		for plat_type in range (4):

			pcap_path = "test"+str(network_type*4+plat_type+1)+".pcapng"
			...
			plt.plot(range(len(cnt_aver_length)), cnt_aver_length,color=color_list[plat_type],label = day1[plat_type])
			res = []
			res.append(('timestamp','CapNum','CapLen'))
			for i in range(300):
			    res.append((i,cnt_total_number[i],cnt_aver_length[i]))

			with open( "test"+str(network_type*4+plat_type+1)+"_net.csv", 'w', newline='') as csvfile:
			    writer = csv.writer(csvfile)
			    writer.writerows(res)

		plt.legend()

		plt.savefig(network_list[network_type] + " real person" + '.pdf',bbox_inches = 'tight')
		# plt.show()