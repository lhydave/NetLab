...
for network_type in range(2):
		...
		for plat_type in range (4):
			...
			plt.plot(range(len(cnt_total_number)), cnt_total_number,color=color_list[plat_type],label = day1[plat_type])

		plt.legend()

		plt.savefig(network_list[network_type] + " real person" + 'PacketNumber.pdf',bbox_inches = 'tight')
		# plt.show()