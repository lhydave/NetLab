...
for network_type in range(2):
	for video_type in range(4):
		...
		for plat_type in range (4):
			...
			plt.plot(range(len(cnt_aver_length)), cnt_aver_length,color=color_list[plat_type],label = plat_list[plat_type])
			res = []
			res.append(('timestamp','CapNum','CapLen'))
			for i in range(300):
			    res.append((i,cnt_total_number[i],cnt_aver_length[i]))

			with open( "test"+str(plat_type*8+offset+9)+"_net.csv", 'w', newline='') as csvfile:
			    writer = csv.writer(csvfile)
			    writer.writerows(res)

		plt.legend()

		plt.savefig(network_list[network_type] + " " + video_list[video_type]+'.pdf')
		# plt.show()